...
package com.x3wiser.anki.provider;
...
import com.x3wiser.anki.BuildConfig;
import com.x3wiser.anki.CollectionHelper;
import com.x3wiser.anki.FlashCardsContract;
import com.x3wiser.anki.FlashCardsContract.CardTemplate;
...
import static com.x3wiser.anki.FlashCardsContract.READ_WRITE_PERMISSION;
...
//                break;
     ...
     *
     * For example: content://com.x3wiser.anki.flashcards/notes?deckId=1234567890123
     *
    ...
    private int bulkInsertNotes(ContentValues[] valuesArr, long deckId) {
        ...
        try {
            ...
            for (int i = 0; i < valuesArr.length; i++) {
                ...
                // Create empty note
                com.x3wiser.libanki.Note newNote = new com.x3wiser.libanki.Note(col, model); // for some reason we cannot pass modelId in here
                // Set fields
                // Check that correct number of flds specified
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case NOTES: {
                ...
                String tags = values.getAsString(FlashCardsContract.Note.TAGS);
                // Create empty note
                com.x3wiser.libanki.Note newNote = new com.x3wiser.libanki.Note(col, col.getModels().get(modelId));
                // Set fields
                String[] fldsArray = Utils.splitFields(flds);
                // Check that correct number of flds specified
                if (fldsArray.length != newNote.getFields().length) {
